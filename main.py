...
def run_workflow(chapter_name, pdf_source):
    ...
    for name, path in paths.items():
        print(f"DEBUG: Attempting to create directory: '{path}' for folder: '{name}'")
        try:
            os.makedirs(path, exist_ok=True)
            print(f"DEBUG: Successfully created/ensured directory: '{path}'")
        except OSError as e:
            print(f"ERROR: Could not create directory '{path}'. Reason: {e}")
            print(f"Please check directory permissions or if the path contains invalid characters.")
            return 
